...
nabpac_sub <- mergeddata %>% filter(arm == 2 , strat2_recep == 2)

# Creating response column 
nabpac_sub$responder_stat <- ifelse(nabpac_sub$bestresp <= 2,"responder","nonresponder")

# Removing patients with no tumor response data
nabpac_sub <- nabpac_sub %>% filter(!is.na(responder_stat))

nabpac_sub_rna <- merge(trans_rna_seq_df,nabpac_sub[c("bestresp","rna_decon_sampleid","responder_stat")],by = "rna_decon_sampleid")
...
# cell_cols <- setdiff(names(nabpac_sub_rna), c("rna_decon_sampleid", "bestresp","responder_stat"))
# 
# # Loop through each gene column
# for (cell in cell_cols) {
#   # Ensure the column is numeric
#   nabpac_sub_rna[[cell]] <- as.numeric(nabpac_sub_rna[[cell]])
#   # Perform Wilcoxon test (Mann-Whitney U test)
#   test_result <- wilcox.test(nabpac_sub_rna[[cell]] ~ nabpac_sub_rna$responder_stat)
# 
#   # Calculate mean expression for each group will be used to determine the dirrection of sig module_results
#   group_means <- nabpac_sub_rna %>%
...
resignature_data <- merge(resignature_data,nabpac_sub[, c("INVESTIGATOR_SAMPLENAME", "responder_stat")],by = "INVESTIGATOR_SAMPLENAME",all.y=TRUE)
...
write_xlsx(module_results, "response_nabpac_modules.xlsx")
...
ge_matrix <- ge_matrix %>% select(1,all_of(nabpac_sub$rna_decon_sampleid))
...
# Removing samples that do not have any RNA data from nabpac_sub
nabpac_sub <-  nabpac_sub %>% filter(rna_decon_sampleid %in% samples_list)
# Removing duplicates
nabpac_sub <- nabpac_sub %>% distinct()

### Metadata for DESeq2
colData <- data.frame(
  condition = nabpac_sub$responder_stat,
...
write_xlsx(res_df, "response_nabpac_DESeq2.xlsx")
...
ggplot(res, aes(x=log2FoldChange, y=-log10(pvalue))) +
  ...
  theme_minimal() +
  labs(title="Nab-paclitaxel Response", x="Log2 Fold Change", y="-Log10(p-value)") +
